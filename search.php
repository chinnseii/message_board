...
<body>
	...
	<h3 align="center"><a href="index.php">戻る</a></h3>
       ...
       <input type="text" class="form-control"placeholder="検索内容を入力してください" name="search"/>
		...
		function news($pageNum=1, $pageSize=5){
            $search_content=$_POST['search'];
			 $array=array();
             require("connect.php");
		    $rs="select message.*,user.imgpath from message,user where user.id=message.id and message.message_content LIKE '%{$_POST['search']}%' order by message.message_id desc limit ".(($pageNum - 1) * $pageSize).",".$pageSize;
		...
		function allNews(){
            $search_content=$_POST['search'];
           require("connect.php");
		    $rs="select count(*) num from message where message_content LIKE '%{$_POST['search']}%'";
		    $r=mysqli_query($conn, $rs);
		...
		foreach($array as $key=>$values){
			...
			echo "<div class='reply{$values->message_id}' style='display:none;width:70%;height:130px;margin-left:10%;margin-top:10px;'>
					<form action='action.php?a=reply&message_id={$values->message_id}' method='post'>
						...
						<input type='submit' value='返事' class='btn btn-success'>
